...
- **Opened BlockRunAI/blockrun#456** to fix the drop itself, since the report alone would have left it standing: one shared module for both NVIDIA legs, image parts forwarded when the catalog tags the model `vision`, still dropped for a text-only one, and now logged on both legs rather than silently on the streaming one. It covers `nvidia/llama-3.2-11b-vision` and three hidden entries; it does **not** cover `nemotron-3-nano-omni`, which is routed through OpenRouter and whose images were never dropped — that one answers as a text-only model only when the cascade substitutes one, which is BlockRunAI/blockrun#450. When #456 deploys, `npm run probe:vision -- --models=nvidia/llama-3.2-11b-vision` re-measures the one visible model for free.
