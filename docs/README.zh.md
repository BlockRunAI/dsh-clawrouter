...
单次价格在约 1000 输入 token 以内是固定的，超过之后会**随上下文增长**。以下是从网关自己的 402 报价实测得到（`deepseek/deepseek-chat`）：
...
也就是说，一个在 10 万 token 上下文里干活的编程智能体，**每次调用大约是下限的 15 倍**。所以当你的平均调用带着很大的上下文时，`/spend` 会**明确提示**这一点，而不是在屏幕上留下一个看着很小、很有底气的数字。它同样看不到「付了钱但失败」的请求。钱包余额才是权威。
...
```sh
npm test          # 175 个离线测试，含两套走真实 cordis Loader 的组合测试
