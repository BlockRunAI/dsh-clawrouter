...
请把它当作**下限**，输出里也是这么写的。链上真正结算的是签名后的 402 报价，而网关算这个报价用的是「预估输入 + `max_tokens`」——是**上限**，不是模型实际生成的量。所以一个 `max_tokens=4096` 但只答了 50 token 的请求，收费远高于实际用量；本计数按实际用量算，就会低这一截。此外只统计**成功完成**的调用，目录没标价的模型会被标为「未计价」而不是当成免费。钱包余额才是权威。

`requestFeeUsd` 默认 `0.002`，因为这是网关真实报的价：一个约 17 token 的请求，402 返回 `{"amount":"0.002000"}`，实测三次调用钱包正好少 $0.006。BlockRun 目前公开的价格页写的是 $0.001。
...
```sh
npm test          # 176 个离线测试，含两套走真实 cordis Loader 的组合测试
